...
The Secret must contain `DATABASE_URL`, `BETTER_AUTH_SECRET`, `SETTINGS_ENCRYPTION_KEY`, and (for a protected unclaimed install) `SETUP_CLAIM_TOKEN`. Keep the encryption key with database backups; replacing it makes saved OIDC, provider, and MCP credentials unreadable.
